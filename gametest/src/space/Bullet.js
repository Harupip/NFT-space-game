export class Bullet {
  ...
  update() {
    ...
    this.y -= this.dy
    if (this.y < -30) { //tăng khả năng trúng
      this.destroy()
      return
    }
  ...
  findEnemies() {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (
        this.y < enemy.y + enemy.height && // thay đổi chính xác theo chiều dọc của đạn
        this.x < enemy.x + enemy.width && //thay đổi chính xác theo chiều ngang của đạn
